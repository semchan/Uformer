...
def calc_rec_loss(reconstructions,label):
    #loss = self.mse_loss(reconstructions.view(reconstructions.size(0), -1), caps_in.view(caps_in.size(0), -1))
    #loss =F.mse_loss(reconstructions.view(reconstructions.size(0), -1), caps_in.view(caps_in.size(0), -1), reduction='none')
    #loss =torch.sqrt(torch.sum((reconstructions.view(reconstructions.size(0), -1)-caps_in.view(caps_in.size(0), -1))**2))
    loss =(torch.cosine_similarity(reconstructions.view(reconstructions.size(0), -1), label.view(label.size(0), -1), dim=-1)+1.0)/2.0
    loss = torch.sum(loss)
